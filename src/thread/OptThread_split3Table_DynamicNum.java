...
public class OptThread_split3Table_DynamicNum extends Thread{
...
//	table1 col 0,1,2,3,4,5,6
//	table2 col 0,2,3,5,7,8,9
//	table3 col 0,2,3,5,10,11,12
	...
	public OptThread_split3Table_DynamicNum(int threadId ,int threadCount , String FolderPath , String site_PathStr , String tableName){
...
//				rowKey 
				...
				HTable table3;
					table3 = new HTable(conf, htd3.getName());
					table3.setAutoFlush(false);
					table3.setWriteBufferSize(1024*1024*2);
				...
				while (reader.readRecord()) { // 逐行读入除表头的数据
					...
					long begin = System.currentTimeMillis();
						...
						Put p3 = new Put(Bytes.toBytes(intToString(i)));
						p3.setWriteToWAL(false);
...
//						table3 col 0,2,3,5,10,11,12
						...
						p2.add(Bytes.toBytes(familyName), Bytes.toBytes(col7), Bytes.toBytes( reader.getValues()[7] ) );
						p2.add(Bytes.toBytes(familyName), Bytes.toBytes(col8), Bytes.toBytes( reader.getValues()[8] ) );
						p2.add(Bytes.toBytes(familyName), Bytes.toBytes(col9), Bytes.toBytes( reader.getValues()[9] ) );
						
						p3.add(Bytes.toBytes(familyName), Bytes.toBytes(col0), Bytes.toBytes( reader.getValues()[0] ) );
						p3.add(Bytes.toBytes(familyName), Bytes.toBytes(col2), Bytes.toBytes( reader.getValues()[2] ) );
						p3.add(Bytes.toBytes(familyName), Bytes.toBytes(col3), Bytes.toBytes( reader.getValues()[3] ) );
						p3.add(Bytes.toBytes(familyName), Bytes.toBytes(col5), Bytes.toBytes( reader.getValues()[5] ) );
						p3.add(Bytes.toBytes(familyName), Bytes.toBytes(col10), Bytes.toBytes( reader.getValues()[10] ) );
						p3.add(Bytes.toBytes(familyName), Bytes.toBytes(col11), Bytes.toBytes( reader.getValues()[11] ) );
						p3.add(Bytes.toBytes(familyName), Bytes.toBytes(col12), Bytes.toBytes( reader.getValues()[12] ) );
